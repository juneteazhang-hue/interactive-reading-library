(() => {
  ...
  function loadProgress() { try { const data = JSON.parse(localStorage.getItem(`reading-progress:${bookId}`)); const validPage = Number.isInteger(data?.page) && data.page >= 0 && data.page <= retellingIndex; return validPage ? { page:data.page, done:Array.isArray(data.done)?data.done:[], flipped:Array.isArray(data?.flipped)?data.flipped.filter(index=>Number.isInteger(index)&&index>=0&&index<8):[] } : {page:0,done:[],flipped:[]}; } catch { return {page:0,done:[],flipped:[]}; } }
  ...
  function renderReading() {
    ...
    const audio=$('#paragraphAudio'),audioStatus=$('#audioStatus');
    audioStatus.hidden=true; audioStatus.textContent='';
    audio.src=`${bookBase}${page.audio}`;
